...
import { money, cents, summary, periodAt, validateExpense } from './finance.js';
import { demoData } from './demo.js';
import { createWorkflows } from './workflows.js';
import { categories } from './categories.js';
import { stockStatus } from './stock-status.js';
import { routes, hrefForPage, pageFromHash } from './routes.js';
...
let demo = false, session = null, page = pageFromHash(location.hash), data = null, busy = false, banner = '';
...
const periodEnd = () => periodAt(data.funds.find(f=>f.kind==='salary')?.starts_on || today(), today()).nextOn;
const workflows=createWorkflows({getData:()=>data,isDemo:()=>demo,db,render,load,esc,money,cents,today,validateExpense,categories,periodEnd});
...
function authScreen(message='') {
 ...
 document.querySelector('#demo').onclick=()=>{if(activating){activation=null;authScreen();return;}demo=true;data=demoData();page=pageFromHash(location.hash);render();};
 document.querySelector('#login')?.addEventListener('submit',async e=>{
  ...
  try{
   ...
   session=auth.session;activation=null;activationError='';localStorage.setItem('home-last-user',username);demo=false;page=pageFromHash(location.hash);await load();
...
function expenseRows(){return [...data.expenses].sort((a,b)=>b.occurred_on.localeCompare(a.occurred_on)).map(e=>`<tr><td>${esc(dateLabel(e.occurred_on))}</td><td><strong>${esc(e.description)}</strong><br><small>${esc(e.category)}</small></td><td>${esc(data.funds.find(f=>f.id===e.fund_id)?.name)}</td><td>${e.method==='card'?'Tarjeta':'Efectivo'}</td><td>${money(e.amount_cents)}</td><td><button class="text" data-expense-stock="${esc(e.id)}">Productos</button><button class="text" data-expense-edit="${esc(e.id)}">Modificar</button><button class="text" data-expense-history="${esc(e.id)}">Historial</button></td></tr>`).join('');}
...
function bills(){return pending().map(r=>`<div class="row"><span class="row-icon">↗</span><div class="row-main"><p><strong>${esc(r.description)}</strong></p><small>${esc(dateLabel(r.due_on))} · ${r.due_on>=periodEnd()?'Próxima quincena · ':'Reservado · '}${esc(data.funds.find(f=>f.id===r.fund_id)?.name)}</small></div><div class="row-amount"><strong>${money(r.amount_cents)}</strong><br><button class="text" data-pay="${esc(r.id)}">Registrar pago</button></div></div>`).join('')||'<div class="empty">No hay pagos pendientes.</div>';}
function dashboard(){const results=data.funds.map(f=>({...f,...summary(f,data.expenses,data.reservations,periodEnd())}));const total=results.reduce((s,f)=>s+f.available,0);return `${data.products.some(p=>stockStatus(p)!=='Disponible')?`<div class="notice">${data.products.some(p=>stockStatus(p)==='Terminado')?'Hay productos terminados. ':''}${data.products.some(p=>stockStatus(p)==='Por terminar')?'Hay productos por terminar. ':''}<a class="text-link" href="#/inventario">Ver inventario</a></div>`:''}<div class="cards"><section class="card primary"><small>Disponible para gastar</small><div class="amount">${money(total)}</div><div class="rule"><span>Reservas ya descontadas</span><span>MXN</span></div></section>${results.map(f=>`<section class="card"><small>${esc(f.name)} · ${f.kind==='voucher'?'Solo tarjeta':'Efectivo y tarjeta'}</small><div class="amount ${f.available<0?'negative':''}">${money(f.available)}</div><div class="rule"><span>Saldo ${money(f.balance)}</span><span>Reservado ${money(f.reserved)}</span></div></section>`).join('')}</div>${results.some(f=>f.available<0)?'<p class="notice">Hay compromisos que superan el saldo disponible. Revisa las reservas y el fondo asignado.</p>':''}<div class="columns"><section class="card"><div class="section-title"><h2>Últimos movimientos</h2><a class="text-link" href="#/movimientos">Ver todos →</a></div>${[...data.expenses].sort((a,b)=>b.occurred_on.localeCompare(a.occurred_on)).slice(0,5).map(e=>`<div class="row"><span class="row-icon">↙</span><div class="row-main"><p><strong>${esc(e.description)}</strong></p><small>${esc(e.category)} · ${esc(dateLabel(e.occurred_on))}</small></div><strong>−${money(e.amount_cents)}</strong></div>`).join('')||'<div class="empty">Registra tu primer gasto para ver tu avance.</div>'}</section><section class="card"><div class="section-title"><h2>Próximos pagos</h2><span class="pill">${pending().length} pendientes</span></div>${bills()}</section></div><footer>El disponible descuenta únicamente las reservas con vencimiento anterior a la próxima quincena. Los pagos posteriores aparecen como próximos, sin reducir el saldo de hoy.</footer>`;}
function render(){if(!data)return;const pages=routes;app.innerHTML=`<div class="layout"><aside class="sidebar"><a class="brand-link" href="#/inicio" aria-label="Ir a Inicio">${brand}</a><div><span class="eyebrow">Mi hogar</span><nav style="margin-top:16px">${pages.map(([p,,i])=>`<a href="${hrefForPage(p)}" class="${page===p?'active':''}" ${page===p?'aria-current="page"':''}><span class="nav-icon">${i}</span>${p}</a>`).join('')}</nav></div><div class="side-foot"><strong>Un poco de orden,<br>más tranquilidad.</strong><p>Finanzas del hogar<br>Inventario y compras · 0.2</p></div></aside><main><div class="topbar"><span>${esc(data.household.name)}<small id="sync-status" class="sync-status">${demo?'Demo local':esc(syncText)}</small></span><div><span class="pill">${demo?'Demostración':'Acceso privado'}</span> <button class="text" id="logout">Salir</button></div></div>${demo?'<div class="notice">Datos ficticios · Esta demostración no guarda ni sincroniza cambios.</div>':''}${banner?`<div class="notice error" role="alert">${esc(banner)}</div>`:''}<div class="heading"><div><span class="eyebrow">Cada decisión cuenta</span><h1>${page==='Inicio'?'Tu hogar, en equilibrio.':esc(page)}</h1><p>${page==='Inicio'?'Una mirada clara a lo que tienes y lo que viene.':'Organiza hoy para disfrutar con tranquilidad.'}</p></div>${['Inicio','Movimientos'].includes(page)?'<button id="add-expense">＋ Registrar gasto</button>':page==='Pagos'?'<button id="add-reservation">＋ Reservar un pago</button>':''}</div><div id="content"></div></main></div>`;
 ...
 else if(page==='Pagos')content.innerHTML=`<p class="notice">Los pagos de la próxima quincena aparecen aquí, pero todavía no reducen el disponible actual. Las recurrencias y los recibos variables se incorporarán en la siguiente etapa.</p><section class="card">${bills()}</section>`;
...
window.addEventListener('hashchange',()=>{if(location.hash.includes('activate=')){activationError='';try{activation=activationFromHash(location.hash);}catch(error){activation=null;activationError=error.message;}history.replaceState(null,'',location.pathname+location.search);demo=false;authScreen();return;}page=pageFromHash(location.hash);banner='';if(data){document.querySelectorAll('dialog').forEach(d=>d.close());render();}});
